...
`run_step4 parse`. Its former `--unsafe-parse-only` behavior is **temporarily disabled** pending the
separately reviewed canonical Step 4 grammar and safe-diagnostic design. Invoking that flag exits
non-zero with `unsafe_parse_only_temporarily_disabled`, reads no raw Step 4 input, and creates no
artifact or stdout diagnostic output. It is not an available diagnostic workflow. Only the normal
validated `run_step4 parse` path may create canonical Step 4 artifacts; never work around the disable
or use the standalone extractor to approve trades; automation must never call it to approve trades.
